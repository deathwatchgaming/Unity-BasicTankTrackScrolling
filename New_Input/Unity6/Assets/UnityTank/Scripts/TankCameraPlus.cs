...
namespace UnityTank.Scripts // Namespace for organization of UnityTank scripts
{
    public class TankCameraPlus : MonoBehaviour // Main camera controller class inheriting from MonoBehaviour
    {
        [Header("References")]
        [Tooltip("The tank rigidbody.")]
        // Reference to the tank's Rigidbody component. Used to read velocity for Follow mode rotation.
        [SerializeField] private Rigidbody tankRigidbody;

        [Tooltip("The tank transform.")]
        // Reference to the tank's Transform. Used as the target for camera positioning and rotation.
        [SerializeField] private Transform tankTransform;

        [Tooltip("The tank camera root transform.")]
        // The camera root transform is used to determine the camera's position and rotation. This is detached from the tank for smooth movement.
        ...
        [Header("Settings")]
        [Tooltip("If tank speed is below this value (in m/s), then the camera will default to looking forward.")]
        ...
        [Tooltip("How closely the camera matches the tank's velocity vector.")]
        // The rotation speed is used to determine how closely the camera matches the tank's velocity vector. The lower the value, the smoother the camera rotations, but too much results in not being able to see where you're going.  
        [SerializeField] private float rotationSpeed = 5.0f;

        [Tooltip("The camera distance determines how closely the camera follows the tank's position.")]
        // The camera distance is used to determine how closely the camera follows the tank's position. The lower the value, the closer the camera is to the tank.
        ...
        [Tooltip("Raycast start height above tank.")]
        ...
        [Tooltip("Safe fallback direction multiplier when very close to tank.")]
        ...
        private Quaternion lookRotation; // The desired rotation of the camera based on the tank's velocity vector.
        private Vector3 desiredPosition; // The desired position of the camera based on the tank's position and the camera's distance and height.
        private Vector3 tankVelocity; // The tank's velocity vector, used to determine the camera's rotation.
        private float desiredHeight; // The desired height of the camera based on the tank's position and the camera's height.
        ...
        private void Awake()
        {
            // Get the tank's rigidbody and transform components
            ...
            {
                Debug.LogWarning("Input Action Asset is not assigned on TankCameraPlus!");
            }
        }
        ...
        private void Update()
        {
            ...
            if (toggleModeAction != null && toggleModeAction.WasPressedThisFrame())
            {
                ...
                if (isOrbitMode)
                {
                    // Robust entry into Orbit mode - calculate proper direction to tank
                    Vector3 toTank = tankTransform.position + Vector3.up * orbitLookAtHeight - cameraTransform.position;
                    float dist = toTank.magnitude;

                    if (dist < minOrbitDistance)
                    {
                        toTank = cameraRoot.forward * fallbackDirectionMultiplier + Vector3.up * fallbackUpComponent;
                    }

                    Quaternion targetRot = Quaternion.LookRotation(toTank.normalized);
        ...
        {
            // Always keep camera root roughly following tank position (shared between modes)
            ...
            if (!isOrbitMode)
            {
                ...
                // If the tank isn't moving (or moving very slowly), default to looking forwards
                // Prevents camera from freaking out with a near-zero velocity getting put into a Quaternion.LookRotation
            ...
            else
            {
                ...
                desiredPosition = tankTransform.position - cameraRoot.forward * clampedDistance;
        ...
        private void LateUpdate()
        {
            ...
            float minSafeY = tankTransform.position.y + minHeightAboveGround;
            ...
            if (isOrbitMode)
            {
                Vector3 lookTarget = tankTransform.position + Vector3.up * orbitLookAtHeight;
        ...
        {
            float safeY = Mathf.Max(cameraTransform.position.y, tankTransform.position.y + height + safeHeightBuffer);
        ...
        {
            Vector3 startPos = tankTransform.position + Vector3.up * raycastStartHeight;
            ...
            {
                float safeY = Mathf.Max(desiredPosition.y, tankTransform.position.y + minHeightAboveGround);
        ...
        {
            Vector3 safeStart = tankTransform.position + Vector3.up * (height + initHeightBuffer);
